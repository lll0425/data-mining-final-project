...
def load_embeddings(which='cifar10', sample=None):
    import h5py
    import os
    import numpy as np
    ...
    try:
        with h5py.File(path, "r") as h5f:
            x = np.array(h5f['embeddings'])  # 特徵向量
            y = np.array(h5f['y_true'])  # 標籤
            labels = ['airplane', 'automobile', 'bird', 'cat', 'deer', 'dog', 'frog', 'horse', 'ship', 'truck']
    except Exception as e:
        raise IOError("Failed to load {}".format(which)) from e

    # 如果 sample 是 'default'，則隨機選取 2000 筆資料
    if sample == 'default':
        idxs = np.random.choice(np.arange(len(y)), 2000, replace=False)
        x, y = x[idxs], y[idxs]
    elif sample is None:
    ...
    # 確保 y 是一維數組
    y = np.squeeze(y)  # 將 (n_samples, 1) 轉換為 (n_samples,)
