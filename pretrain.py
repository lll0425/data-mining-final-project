...
## Adding CIFAR-10 to dataset options
def make_dkmeans_cifar10(input_shape, n_embeddings=10):
    import tensorflow as tf
    from tensorflow.keras.layers import Dense, Flatten, Reshape
    from tensorflow import random_normal_initializer
    from operator import mul
...
def __normalize_keras_dataset(train, test):
    ...
    # Normalize CIFAR-10 to [0, 1]
    x_train = x_train.astype(np.float32) / 255
    x_test = x_test.astype(np.float32) / 255

    # If the images are grayscale, expand the dimensions to make it 3D
    if len(x_train.shape) < 4:
        x_train = np.expand_dims(x_train, -1)
        x_test = np.expand_dims(x_test, -1)
...
def load_cifar10_normalized():
    from tensorflow.keras.datasets import cifar10

    return __normalize_keras_dataset(*cifar10.load_data())


def make_cifar10_dataset():
    from Util.Datasets import Dataset

    train, test = load_cifar10_normalized()
    labels = [str(i) for i in range(10)]
...
def main():
    ...
    datasets = {
        ...
        'cifar10': make_cifar10_dataset,  # Adding CIFAR-10 to dataset options
    ...
    log_folder = os.path.join('tb_logs', args.data, datetime.datetime.now().isoformat().replace(":", "-"))
    summary_writer = tf.summary.create_file_writer(log_folder)

    dataset = datasets[args.data]()
    train_x, train_y = dataset.train
    if args.data == 'cifar10':
        builder = make_dkmeans_cifar10(train_x.shape[1:], n_embeddings=args.embeddings)
    else:
        builder = make_dkmeans_mnist(train_x.shape[1:], n_embeddings=args.embeddings)  # 默認使用 mnist 模型
